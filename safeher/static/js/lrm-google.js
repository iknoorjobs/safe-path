...
L.Routing.Google = L.Class.extend({
    ...
    _decodePolyline: function(geometry) {
            var decoded_latlngs = google.maps.geometry.encoding.decodePath(geometry);
            var leaf_latlngs = [];
            for (var i = 0; i < decoded_latlngs.length; i++) {
                leaf_latlngs.push(L.latLng(decoded_latlngs[i].lat(),decoded_latlngs[i].lng())); 
            }
   //          console.log(leaf_latlngs); 
			// var coords = L.polyline.decode(geometry, 5),
